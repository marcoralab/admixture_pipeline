---
title: "Admixture filtering for LAI Reference"
...
# Processing
Before interpreting the results from ADMIXTURE, we did the following:
...
Before labeling clusters, Middle Eastern references are removed. The checked superpopulation labels are used for this labeling process.

Each cluster is assigned a superpopulation based on what superpopulation that cluster has the highest average proportion within.
...
plot_admix <- function(x, by = cluster_cols, poplabel = TRUE) {
  ...
  if (!poplabel) {
    ...
    tbl_use <- tbl_use |>
      ...
      left_join(pop_number, by = "IID") |>
      mutate(spop = label_n,
             IID = forcats::fct_inorder(as.character(IID)))
    stripangle <- 0
  }

  plt <- ggplot(tbl_use, aes(x = IID, y = prop, fill = Cluster)) +
